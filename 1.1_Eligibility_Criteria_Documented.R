################################################################################
# Script: 1.1.Eligibility_Criteria.R
#
# Purpose:
#   Defines eligibility criteria and cohort construction for older adult and
#   pregnant populations included in the influenza and COVID-19 vaccine cohorts.
#
# Main sections:
#   1. Load demographic data
#   2. Construct older-adult influenza cohort
#   3. Construct older-adult COVID-19 cohort
#   4. Exclude prior COVID-19 diagnoses, tests, and vaccinations
#   5. Construct pregnancy cohorts for influenza and COVID-19
################################################################################
...
library(dplyr)
library(haven)
SSB <- read_sas("Path_To_SSB_File.sas7bdat")
...
npr_som <- read_delim("Path_to_NPR_File.csv", 
...
merged$test <- as.Date(merged$ref_date) + merged$PrvedatoDiffDager
...
library(haven)
mbrn <- read_sav("Path_to_p222359_mfr_2_2023_2.sav")
mbrn <- mbrn[!is.na(mbrn$SVLEN_DG),]  # removes 2,372
reference_FHI <- read.csv("Path_to_Reference_dates_FHI.csv")
