...
apply_iptw <- function(data) {
  ...
  w <- weightit(mental ~ age_at_enrollment_categorized + county_2021 + country_of_birth + Marital_status + BMI + Education
...
apply_iptw <- function(data) {
  # Calculate weights (example: ATE weights for binary treatment)
  w <- weightit(mental ~ age_at_enrollment_categorized + county_2017 + country_of_birth + Marital_status + BMI + Education
...
apply_iptw <- function(data) {
  # Calculate weights (example: ATE weights for binary treatment)
  w <- weightit(any_mh ~ age_at_enrollment_categorized + county_2017 + country_of_birth + Marital_status + Dementia + Education
...
apply_iptw <- function(data) {
  # Calculate weights (example: ATE weights for binary treatment)
  w <- weightit(mental ~ age_at_enrollment_categorized + county_2021 + country_of_birth + Marital_status + Dementia + Education
...
save(old_covid_imp, file = "Old_COVID_Weights_SA_Cov.rdata")
##########################################################################################
...
save(old_flu_imp_iptw, file = "Old_Influenza_Weight_Corrected_SA_Cov.rdata")
...
save(preg_flu_risk_imp_iptw, file = "Pregnant_Influenza_Risk_Weight_Corrected_SA_Cov.rdata")
###################################
# Pregnant flu norisk
# Step 1: Create a function to trim weights at 1st and 99th percentiles
trim_weights <- function(w.out, lower_percentile = 0.01, upper_percentile = 0.99) {
...
save(preg_flu_norisk_imp_iptw, file = "Pregnant_Influenza_NoRisk_Weight_Corrected_SA_Cov.rdata")
############################################3
...
save(preg_covid_norisk_imp_iptw, file = "Pregnant_COVID_NoRisk_Weight_Corrected_SA_Cov.rdata")
##################################3
# Pregnant covid risk
# Step 1: Create a function to trim weights at 1st and 99th percentiles
trim_weights <- function(w.out, lower_percentile = 0.01, upper_percentile = 0.99) {
...
save(preg_covid_risk_imp_iptw, file = "Pregnant_COVID_Risk_Weight_Corrected_SA_Cov.rdata")
##################################################################################
# Step 1: Create a function to trim weights at 1st and 99th percentiles
trim_weights <- function(w.out, lower_percentile = 0.01, upper_percentile = 0.99) {
  # Extract original weights
...
weight_flags_before <- data.frame(
  imputation = 1:25,
  max_gt_10 = sapply(old_covid_imp, function(x) max(x$iptw_weights) > 10),
  max_mean_ratio = sapply(old_covid_imp, function(x) max(x$iptw_weights) / mean(x$iptw_weights)),
  cv = sapply(old_covid_imp, function(x) sd(x$iptw_weights) / mean(x$iptw_weights)),
  ess = sapply(old_covid_imp, function(x) (sum(x$iptw_weights)^2) / sum(x$iptw_weights^2)),
  n = sapply(old_covid_imp, nrow)
)
...
save(old_covid_imp_iptw, file = "Old_COVID19_Weight_Corrected_SA_Cov.rdata")
