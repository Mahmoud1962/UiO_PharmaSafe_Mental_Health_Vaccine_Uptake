...
(length(rows_missing)/1877) * 100  #### We need 50 imputations
...
(length(rows_missing)/135172) * 100
...
predictor_matrix[,"income_2021_cat"]<- 1
...
cov_preg_covid_risk$Education <- as.factor(cov_preg_covid_risk$Education)
cov_preg_covid_risk$BMI <- as.factor(cov_preg_covid_risk$BMI)
cov_preg_covid_risk$Marital_status <- as.factor(cov_preg_covid_risk$Marital_status)

method <- make.method(cov_preg_covid_risk) 


imp_preg_covid_risk <- mice(cov_preg_covid_risk, m= 50, maxit = 1, method = method, predictorMatrix = predictor_matrix, seed = 123)

saveRDS(imp_preg_covid_risk, file = "Cov_Imputed_Pregnancy_Covid_Risk_SA_Cov.rds")
...
cov_preg_covid_norisk$Education <- as.factor(cov_preg_covid_norisk$Education)
cov_preg_covid_norisk$BMI <- as.factor(cov_preg_covid_norisk$BMI)
cov_preg_covid_norisk$Marital_status <- as.factor(cov_preg_covid_norisk$Marital_status)

method <- make.method(cov_preg_covid_norisk) 


imp_preg_covid_norisk <- mice(cov_preg_covid_norisk, m= 50, maxit = 1, method = method, predictorMatrix = predictor_matrix, seed = 123)

saveRDS(imp_preg_covid_norisk, file = "Cov_Imputed_Pregnancy_Covid_Norisk_SA_Cov.rds")
...
cov_preg_flu_norisk$Education <- as.factor(cov_preg_flu_norisk$Education)
cov_preg_flu_norisk$BMI <- as.factor(cov_preg_flu_norisk$BMI)
cov_preg_flu_norisk$Marital_status <- as.factor(cov_preg_flu_norisk$Marital_status)

method <- make.method(cov_preg_flu_norisk) 


imp_preg_flu_norisk <- mice(cov_preg_flu_norisk, m= 50, maxit = 1, method = method, predictorMatrix = predictor_matrix, seed = 123)

saveRDS(imp_preg_flu_norisk, file = "Cov_Imputed_Pregnancy_Influenza_Norisk_SA_Cov.rds")
...
cov_preg_flu_risk$Education <- as.factor(cov_preg_flu_risk$Education)
cov_preg_flu_risk$BMI <- as.factor(cov_preg_flu_risk$BMI)
cov_preg_flu_risk$Marital_status <- as.factor(cov_preg_flu_risk$Marital_status)

method <- make.method(cov_preg_flu_risk) 


imp_preg_flu_risk <- mice(cov_preg_flu_risk, m= 50, maxit = 1, method = method, predictorMatrix = predictor_matrix, seed = 123)
...
cov_old_covid <- cov_old_covid %>% group_by(person_id) %>%
  mutate(age_at_enrollment_categorized = coalesce(age_at_enrollment_categorized, na.omit(age_at_enrollment_categorized)[1]),
         county_2021 = coalesce(county_2021, na.omit(county_2021)[1]),
         country_of_birth = coalesce(country_of_birth, na.omit(country_of_birth)[1]),
...
cov_old_covid$Dementia <- as.factor(cov_old_covid$Dementia)
cov_old_covid$Education <- as.factor(cov_old_covid$Education)
cov_old_covid$Marital_status <- as.factor(cov_old_covid$Marital_status)

method <- make.method(cov_old_covid) 


imp_old_covid <- mice(cov_old_covid, m= 25, maxit = 1, method = method, predictorMatrix = predictor_matrix, seed = 123)
...
cov_old_flu$Dementia <- as.factor(cov_old_flu$Dementia)
cov_old_flu$Education <- as.factor(cov_old_flu$Education)
cov_old_flu$Marital_status <- as.factor(cov_old_flu$Marital_status)
...
saveRDS(imp_old_flu, file = "Cov_Imputed_Old_Influenza_SA_Cov.rds")
